...
public interface SaleRepository extends JpaRepository<Sale, Long> {
    ...
    @Query(value = "SELECT obj FROM Sale obj JOIN FETCH obj.seller " +
            "WHERE obj.date > :minDate AND obj.date < :maxDate AND " +
            "UPPER(obj.seller.name) LIKE UPPER(CONCAT('%', :name, '%'))",
            countQuery = "SELECT COUNT(obj) FROM Sale obj JOIN obj.seller " +
                    "WHERE obj.date > :minDate AND obj.date < :maxDate AND " +
                    "UPPER(obj.seller.name) LIKE UPPER(CONCAT('%', :name, '%'))")
    Page<Sale> searchAllReports(LocalDate minDate, LocalDate maxDate, String name, Pageable pageable);
    ...
    List<SaleSummaryDTO> searchAllSummary(LocalDate minDate, LocalDate maxDate);
}
